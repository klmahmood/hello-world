...
using namespace std;
typedef int L;
int intDigits(int x) { return (int)log10(abs(x)) + 1; }
L gcd(L a, L b) {
  L c;
  while (a != 0) {
    c = a;
...
int main() {
  ...
  while (cin >> n, n) {
    ...
    } else  // avg is not integer
    {
      ...
      if (div >= 1) {
        if (isNeg) {
          cout << "  " << setfill(' ') << setw(intDigits(div) + intDigits(den))
               << num << endl;
          cout << "- " << div;
          for (i = 0; i < intDigits(den); i++) cout << "-";
          cout << endl;
          cout << "  ";                         // space for -
          ...
          cout << setfill(' ') << setw(intDigits(div) + intDigits(den)) << num
               << endl;
          cout << div;
          for (i = 0; i < intDigits(den); i++) cout << "-";
          cout << endl;
